...
#    C -= np.diag(np.array(C.diagonal()).flatten())
    ...
    if False: 
        ...
        grammatical_open = ["JJ_NN"]
        ungrammatical_open = [""]
        grammatical_close=["PPRS_NN", "DT_NN"]
        ungrammatical_close=[""]
        ...
        for idx_gram in range(len(grammatical)):
            for idx_ugram in range(len(ungrammatical)):
                ...
                f = open("../rsc/bigrams/" + grammatical[idx_gram] + ".txt", "r")
                setG = f.readlines()
                f.close()

                f = open("../rsc/bigrams/" + grammatical[idx_gram] + "_frqs.txt", "r")
                setG_frqs = f.readlines()
                f.close()

                f = open("../rsc/bigrams/" + ungrammatical[idx_ugram] + ".txt", "r")
                setUG=f.readlines()
                f.close()
                ...
                for k in range(len(setG[:NSamples])):
                    correct = setG[k]
                    frq = setG_frqs[k]
                    for l in range(len(setUG[:NSamples])):
                        incorrect = setUG[l]
