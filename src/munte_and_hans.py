...
#    C -= np.diag(np.array(C.diagonal()).flatten())
    ...
    comp_sets = {"verbs":  {"g":"PPR_VBP","ug":"PPRS_VBP"}, 
                 "nouns": {"g":"PPRS_NN", "ug":"PPR_NN"}}
    ...
    if True:
        ...
        for group in comp_sets.keys():
            print(group)
            grammatical   = comp_sets[group]["g"] 
            ungrammatical = comp_sets[group]["ug"]


            #pair_set = pairs[sweep_idx]
            #pair_set = grammatical[sweep_idx_i] + "_2_" + ungrammatical[sweep_idx_j]
            pair_set = grammatical + "_2_" + ungrammatical
            print(pair_set)

            f = open("../rsc/bigrams/" + grammatical + ".txt", "r")
            setG = f.readlines()
            f.close()

            f = open("../rsc/bigrams/" + grammatical + "_frqs.txt", "r")
            setG_frqs = f.readlines()
            f.close()

            f = open("../rsc/bigrams/" + ungrammatical + ".txt", "r")
            setUG=f.readlines()
            f.close()
            ...
            for k in range(len(setG[:NSamples])):
                correct = setG[k]
                frq = setG_frqs[k]
