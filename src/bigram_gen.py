...
if __name__ == "__main__":
    ...
    hparams = {"bank_labels":["t-{}".format(i) for i in xrange(K)],
               "eps":0.0000001, 
               "eta":1,
               "alpha":1,
               "beta":1,
    ...
    if MODE == "help":
        print "Counts are broken up into chunks and the sweep idx is for breaking up a task into parallel parts"
        print '\n'.join(["run <Mem Path> <Sweep IDX> <nchunk> <totalchunks>"])
    ...
    if MODE == "run":
        ...
        vocab = f.readlines()
        ANet.vocab = [vocab[i].strip() for i in xrange(len(vocab))]
        f.close()
        ANet.I = {ANet.vocab[i]:i for i in xrange(len(vocab))}
...
#        C -= np.diag(np.array(C.diagonal()).flatten())
        ...
        for k in xrange(len(pair_items[:NSamples])):
