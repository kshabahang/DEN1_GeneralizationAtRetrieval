    ...
    def eval(self, verbose=False):
        ...
        for cond in self.QaAs.keys():
            ...
            for i in xrange(len(self.QaAs[cond])):
                ...
                self.idx_predict = q.split().index("#") + 1
    ...
    def update_vocab(self, corpus_process):
        ...
        for i in xrange(len(corpus_process)):
            if corpus_process[i] in self.I:
                if corpus_process[i] not in self.wf:
        ...
        if self.V > self.V0:
            print ("Warning: vocabulary size exceeded {} word limit".format(self.V0))
...
#                       assert o[self.I[input_sentence[iShiftS]]] == 1, 'iShiftS {}; iShiftK {}'.format(iShiftS, iShiftK)
    ...
    def load_vocab(self, path):
        ...
        self.V = len(self.I)
    def probe(self, sentence, showRec = False, st = 1.0, norm = False, noise = 0):
        X_order, X_syntag = self.sent2vec(sentence.split())
        if norm:
            X_order = X_order + np.abs(np.random.normal(0, noise, len(X_order)))
            X_syntag = X_syntag + np.abs(np.random.normal(0, noise, len(X_syntag)))

        X_order  = X_order/np.linalg.norm(X_order)
        X_syntag = X_syntag/np.linalg.norm(X_syntag)

        report = self.report(st*X_order, st*X_syntag)
        if showRec:
    ...
    def sort_banks(self, echo = None):
        ...
        if type(echo) != ndarray:
            ...
            elif self.hparams["mode"] == "numpy":
                echo = deepcopy(self.echo_full)
        #print echo
